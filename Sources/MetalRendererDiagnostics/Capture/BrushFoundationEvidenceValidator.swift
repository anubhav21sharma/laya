...
public enum BrushFoundationCompilerProbe {
    ...
    public static func capture(
        ...
        for index in 0..<logicalDabEvaluationCount {
            ...
            repeat {
                let page = try cursor.emitNextPage { _ in
                    emitted += 1
                }
                precondition(page.emittedCount <= LogicalDabBatch.maximumDabCount)
            } while !cursor.isComplete
            guard cursor.completedGenerator != nil else {
                preconditionFailure(
                    "Completed compiler probe cursor has no generator"
                )
            }
        }
