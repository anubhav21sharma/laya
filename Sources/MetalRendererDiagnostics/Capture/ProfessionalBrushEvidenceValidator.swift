...
public enum ProfessionalBrushEvidenceValidator {
    ...
    public static func validate(
        ...
        else { throw invalid("professional scene schema is not current") }
        guard let requirement = sceneRequirements[evidence.scene]
        else { throw invalid("professional scene identity is unknown") }
        guard evidence.family == requirement.family,
              evidence.definitionID == requirement.definitionID
        else { throw invalid("professional family or definition identity is invalid") }
        guard isSHA256(evidence.definitionSemanticHash)
        else { throw invalid("professional definition semantic hash is invalid") }
        guard isCurrentPipelineKey(evidence.pipelineKey),
              evidence.abiVersion == DepositionABI.version
        else { throw invalid("professional pipeline or ABI identity is invalid") }
        guard evidence.residentResourceBytes == requirement.residentBytes
        else {
            throw invalid(
                "professional resident resource bytes are invalid: "
                    + "\(evidence.residentResourceBytes)"
            )
        }
        guard evidence.logicalDabCount > 0,
              evidence.projectedInstanceCount >= evidence.logicalDabCount
        else { throw invalid("professional logical or projected counts are invalid") }
        guard [
        ...
        else { throw invalid("professional artifact SHA-256 identity is invalid") }
        guard evidence.previewCommitMaximumChannelDelta <= 1
        else {
            throw invalid(
                "professional preview/commit delta is invalid: "
    ...
    private static func isCurrentPipelineKey(_ value: String) -> Bool {
        ...
        return fields.count == 9
            && fields[0] == "deposition"
            && fields[6] == "abi\(DepositionABI.version)"
            && fields[7]
