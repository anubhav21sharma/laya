...
enum CompositeHarnessControlMutation: String, CaseIterable, Sendable {
    case sharedRandomStream
    case globalComponentRegroup
    case retainedSecondaryReplay
    case dynamicComponentStorage
    case omittedSecondaryBounds
    case omittedSecondaryResources

    var invariantName: String {
        switch self {
        case .sharedRandomStream:
            "compositeRandomIsolationControl"
        case .globalComponentRegroup:
            "compositeAuthoredOrderControl"
        case .retainedSecondaryReplay:
            "compositeRetainedReplayControl"
        case .dynamicComponentStorage:
            "compositeFixedStorageControl"
        case .omittedSecondaryBounds:
            "compositeAggregateBoundsControl"
        case .omittedSecondaryResources:
            "compositeAggregateResourceControl"
        }
    }
}
...
public final class DepositionHarnessRunner {
    ...
    private let compositeControlMutation: CompositeHarnessControlMutation?
    ...
    init(
        ...
        library: any MTLLibrary,
        productionAnchorDefinitions: [String: BrushDefinition],
        compositeControlMutation: CompositeHarnessControlMutation
    ) {
        self.device = device
        self.library = library
        self.productionAnchorDefinitions = productionAnchorDefinitions
        self.compositeControlMutation = compositeControlMutation
    }
    ...
    public func run(
        ...
        if stem == "deposition-ink" {
            cpuReference = Self.hardRoundCPUReference(
                ...
                material:
                    context.compiled.primaryComponent.depositionMaterial,
                width: scene.width,
                height: scene.height
            )
        } else {
        ...
        let textures = combinedBrushTextures(context.compiled)
        let textureLevels = Dictionary(
            uniqueKeysWithValues: textures.keys.sorted().map {
                ($0, textures[$0]!.mipmapLevelCount)
            }
        )
        let evidence = DepositionSceneEvidence(
            ...
            pipelineKey: Self.pipelineDescription(
                context.compiled.primaryComponent.depositionPipeline.key
            ),
            abiVersion:
                context.compiled.primaryComponent.depositionPipeline.key
                    .abiVersion,
...
private extension DepositionHarnessRunner {
    ...
    func makeContext(
        ...
        if package.definition.components.allSatisfy({
            $0.material.accumulation == .destinationOut
        }) {
    ...
    func performStroke(
        ...
        let compositeMode: StrokeCompositeMode = context.compiled.program
            .definition.components.allSatisfy {
                $0.material.accumulation == .destinationOut
            } ? .erase : .draw
        if zoom != 1 {
        ...
        let style = StrokeRenderStyle(
            ...
            componentRandomNamespaceMode:
                stem == "deposition-composite"
                    && compositeControlMutation == .sharedRandomStream
                    ? .sharedPrimary : .isolated
        )
        ...
        {
            let beganResult = try await renderer
        ...
        var terminalFlushes: [HarnessLiveFlushResult] = []
        if !cancel {
            ...
            terminalFlushes = try await renderer
                .drainPreparedStrokeInputForHarness(
                    outputPixelSize: renderer.pixelSize
                )
        }
        let finalFlush: HarnessLiveFlushResult
        if let lastTerminalFlush = terminalFlushes.last {
            finalFlush = lastTerminalFlush.mergingPrecedingSubmissions(
                drainedFlushes + terminalFlushes.dropLast()
            )
        } else {
            finalFlush = try await renderer.flushPendingLiveForHarness()
                .mergingPrecedingSubmissions(drainedFlushes)
        }
        let flushMetrics = finalFlush.metrics
        if collectPerformanceEvidence {
            try recordPerformanceFlush(
                finalFlush,
        ...
        let liveFrame = finalFlush.frame
        let commitMetrics: GPUFrameMetrics
        if cancel {
...
@MainActor
private func combinedBrushTextures(
    _ brush: CompiledBrush
) -> [String: any MTLTexture] {
    var textures = brush.primaryComponent.textures
    guard let secondary = brush.secondaryComponent else { return textures }
    for (identifier, texture) in secondary.textures {
        if let existing = textures[identifier] {
            precondition(
                ObjectIdentifier(existing as AnyObject)
...
private extension DepositionHarnessRunner {
    func professionalInvariantResults(
        ...
        let actualResources = Dictionary(
            uniqueKeysWithValues: combinedBrushTextures(context.compiled).map {
    ...
    func finishProfessionalRun(
        ...
        let resolvedResources =
            textures.keys.sorted().map { identity in
                ProfessionalBrushResolvedResource(
                    ...
                    kind: identity.hasPrefix("builtin.shape.")
                        ...
                        : "grain",
                    mipCount:
                        textures[identity]!.mipmapLevelCount
                )
            }
    ...
    func professionalPerformanceArtifacts(
        ...
        let zeroEventPhases = zip(
            eventToSubmitNanoseconds,
            capture.identityFrames
        ).compactMap { duration, frame in
            duration == 0 ? frame.inputPhase : nil
        }
        let finalIdentityFrame = capture.identityFrames.last
        let finalLogicalIdentityHighWater = finalIdentityFrame.flatMap {
            Int(exactly: $0.emittedLogicalDabHighWater)
        }
        ...
        guard capture.strokeMetrics.count == 128,
              ...
              let finalIdentityFrame,
              let finalLogicalIdentityHighWater,
            ...
            throw DepositionHarnessRunError.invariantFailed(
                scene: scene.name,
                invariant:
                    "professionalLongStrokeEvidenceComplete("
                    + "metrics=\(capture.strokeMetrics.count),"
                    + "identities=\(capture.identityFrames.count),"
                    + "final=\(finalIdentityFrame != nil),"
                    + "representable=\(finalLogicalIdentityHighWater != nil),"
                    + "events=\(eventToSubmitNanoseconds.count),"
                    + "zeroEvents=\(eventToSubmitNanoseconds.filter { $0 == 0 }.count),"
                    + "zeroPhases=\(zeroEventPhases),"
                    + "limits=\(limits != nil),trend=\(trend != nil))"
            )
        }
    ...
    func measureProfessionalFiveHundredDabs(
        ...
        for _ in 0..<3 {
            ...
            let prepared = try encoder.preflight(
                records: exact,
                binding:
                    context.compiled.primaryComponent.depositionPipeline,
                material:
                    context.compiled.primaryComponent.depositionMaterial,
                target: target
            )
    ...
    func invariantResults(
        ...
        if let productionDefinition = productionAnchorDefinitions[stem] {
            ...
            results["familyAndAccumulationCorrect"] =
                expected.map {
                    ...
                    return definition.id.rawValue == $0.definitionID
                        && component.material.accumulation == $0.accumulation
                        && component.material.edgeTreatment == $0.edge
                        && component.coverage.shapes.first?.shape == $0.shape
                        && component.placement.baseFlow == $0.flow
                } ?? true
        }
        ...
        case "deposition-composite":
            results["compositeTwoComponentContract"] =
                compositeTwoComponentContract(
                    capture: primary,
                    compiled: context.compiled
                )
            results.merge(compositeControlResults(
                capture: primary,
                compiled: context.compiled,
                mutation: compositeControlMutation
    ...
    func compositeTwoComponentContract(
        capture: StrokeCapture,
        compiled: CompiledBrush
    ) -> Bool {
        let definition = compiled.program.definition
        guard definition.composition == .orderedSourceOver,
              definition.components.count == 2,
              definition.components[0].identifier.rawValue == "dry-body",
              definition.components[1].identifier.rawValue == "dry-detail",
              definition.components[0].ordinal == 0,
              definition.components[1].ordinal == 1,
              compiled.primaryComponent.ordinal == 0,
              compiled.secondaryComponent?.ordinal == 1,
              compiled.residentByteCount > 0
        else { return false }

        let primaryRecords = capture.scheduledRecords.filter {
            $0.componentOrdinal == 0
        }
        let secondaryRecords = capture.scheduledRecords.filter {
            $0.componentOrdinal == 1
        }
        guard !primaryRecords.isEmpty,
              !secondaryRecords.isEmpty,
              primaryRecords.count != secondaryRecords.count,
              capture.scheduledRecords.first?.componentOrdinal == 0,
              let sharedPrimary = compiled.primaryComponent
                .textures[DepositionHarnessFixtures.compositeSharedGrainID],
              let sharedSecondary = compiled.secondaryComponent?
                .textures[DepositionHarnessFixtures.compositeSharedGrainID],
              ObjectIdentifier(sharedPrimary as AnyObject)
                == ObjectIdentifier(sharedSecondary as AnyObject)
        else { return false }

        let primaryColor = primaryRecords[0].instance.premultipliedColor
        let secondaryColor = secondaryRecords[0].instance.premultipliedColor
        return primaryColor != secondaryColor
            && definition.components[0].placement.baseSpacingFraction
                != definition.components[1].placement.baseSpacingFraction
            && definition.components[0].sensorProgram
                != definition.components[1].sensorProgram
    }

    func compositeControlResults(
        capture: StrokeCapture,
        compiled: CompiledBrush,
        mutation: CompositeHarnessControlMutation?
    ) -> [String: Bool] {
        let records = capture.scheduledRecords
        let fixtureComponentDigest =
            "b2cdbbb4ffb5f7d5e4741ec3cf45b9af9dedb64262158ed65faebeccd7df0e93"
        let fixtureIdentityDigest =
            "aa9383bfd974894c9cfce4ba74281b0b9d93a080d3acb9ca818ef9be6dfe6d29"
        let fixtureRandomOutputDigest =
            "5c9ca7891437d60ecd53061b44b743847ad140b5a3e47e95cadc34e6d294d5de"
        let observedComponents = mutation == .globalComponentRegroup
            ? records.map(\.componentOrdinal).sorted()
            : records.map(\.componentOrdinal)
        let observedIdentities = mutation == .retainedSecondaryReplay
            ? records.map(\.identity) + records.lazy.filter {
                $0.componentOrdinal == 1
            }.map(\.identity)
            : records.map(\.identity)
        let componentDigest = DepositionSceneEvidence.sha256(
            observedComponents
        )
        let identityDigest = DepositionSceneEvidence.sha256(
            observedIdentities.flatMap { identity in
                withUnsafeBytes(of: identity.littleEndian, Array.init)
            }
        )

        let fixedCompiledStorageLabels: Set<String> = [
            "program", "backendContract", "renderIdentity",
            "primaryComponent", "secondaryComponent", "residentByteCount",
            "report", "diagnostics", "cacheKeys",
        ]
        let actualCompiledStorageLabels = Set(
            Mirror(reflecting: compiled).children.compactMap(\.label)
        )
        let fixedStorage = mutation != .dynamicComponentStorage
            && actualCompiledStorageLabels == fixedCompiledStorageLabels
            && compiled.primaryComponent.ordinal == 0
            && compiled.secondaryComponent?.ordinal == 1

        let auditedProjectedCount = mutation == .omittedSecondaryBounds
            ? records.lazy.filter { $0.componentOrdinal == 0 }.count
            : capture.projectedInstanceCount

        let fixtureResources: Set<String> = [
            "composite.primary.shape",
            "composite.secondary.shape",
            DepositionHarnessFixtures.compositeSharedGrainID,
        ]
        let auditedResources = mutation == .omittedSecondaryResources
            ? Set(compiled.primaryComponent.textures.keys)
            : Set(combinedBrushTextures(compiled).keys)

        var randomOutputBytes: [UInt8] = []
        randomOutputBytes.reserveCapacity(records.count * 33)
        for record in records {
            randomOutputBytes.append(record.componentOrdinal)
            for value in [
                record.instance.premultipliedColor.x,
                record.instance.premultipliedColor.y,
                record.instance.premultipliedColor.z,
                record.instance.premultipliedColor.w,
                record.instance.coverageInputs.x,
                record.instance.coverageInputs.y,
                record.instance.coverageInputs.z,
                record.instance.coverageInputs.w,
            ] {
                var bits = value.bitPattern.littleEndian
                withUnsafeBytes(of: &bits) {
                    randomOutputBytes.append(contentsOf: $0)
                }
            }
        }
        let randomOutputDigest = DepositionSceneEvidence.sha256(
            randomOutputBytes
        )

        return [
            "compositeAggregateBoundsControl":
                capture.logicalDabCount == 89
                    && auditedProjectedCount == 89
                    && records.count == 89
                    && records.lazy.filter {
                        $0.componentOrdinal == 0
                    }.count == 57
            ...
            "compositeAggregateResourceControl":
                auditedResources == fixtureResources
                    && compiled.residentByteCount == 16_383,
            "compositeAuthoredOrderControl":
                componentDigest == fixtureComponentDigest,
            "compositeFixedStorageControl": fixedStorage,
            "compositeRandomIsolationControl":
                randomOutputDigest == fixtureRandomOutputDigest,
            "compositeRetainedReplayControl":
                identityDigest == fixtureIdentityDigest,
        ]
    }
    ...
    func tilingTranslationIsEqual(
        ...
        guard Self.maximumChannelDelta(first, second) <= 1 else {
            return false
        }
    ...
    func customTexturesAreExact(_ brush: CompiledBrush) -> Bool {
        ...
        let component = brush.primaryComponent
        return Set(component.textures.keys)
                == Set(expected.map(\.0))
            && expected.allSatisfy { id, slot in
                guard let compiledTexture = component.textures[id],
                      let bound =
                        component.depositionMaterial.textures[slot]
                else {
                    return false
                }
        ...
        for combination in [
            ...
            for twoShapes in [false, true] {
                for grainCount in 0...2 {
                    ...
                    let coverage = context.compiled.program
                        .primaryComponent.definition.coverage
                    sawOneShape =
                    ...
                    resourceSlotsAreExact =
                        resourceSlotsAreExact
                        && Set(
                            context.compiled.primaryComponent
                                .depositionMaterial
                                .textures.boundSlots
                        ) == expectedSlots
                    guard context.compiled.primaryComponent.pipelineKey
                            .functionConstants.usesSecondaryShape
                            == twoShapes,
                          context.compiled.primaryComponent.pipelineKey
    ...
    func mipMatrixIsComplete(scene: HarnessScene) async throws -> Bool {
        ...
        guard let levels = combinedBrushTextures(context.compiled).values
...
private extension DepositionHarnessRunner {
    ...
    func directlyEncodedBytes<S: Sequence>(
        ...
        let prepared = try encoder.preflight(
            records: values,
            binding: compiled.primaryComponent.depositionPipeline,
            material: compiled.primaryComponent.depositionMaterial,
            target: target
        )
        _ = try encoder.encode(
            prepared,
...
private extension DepositionHarnessRunner {
    ...
    static func encodedBGRA8Bytes(
        ...
        for offset in stride(from: 0, to: pixels.count, by: 4) {
            let red = float32(binary16Bits: pixels[offset])
            let green = float32(binary16Bits: pixels[offset + 1])
            let blue = float32(binary16Bits: pixels[offset + 2])
            let alpha = min(
                1,
                max(0, float32(binary16Bits: pixels[offset + 3]))
            )
            let color = LinearPremultipliedColor(
                red: min(alpha, max(0, red)),
                green: min(alpha, max(0, green)),
                blue: min(alpha, max(0, blue)),
                alpha: alpha
            )!
    ...
    static func float32(binary16Bits bits: UInt16) -> Float {
...
enum DepositionHarnessFixtures {
    ...
    static func compositePackage() throws -> BrushPackage {
        let primaryShapeID = "composite.primary.shape"
        let secondaryShapeID = "composite.secondary.shape"
        let primaryShapeData = try asymmetricPNG(seed: 29)
        let secondaryShapeData = try asymmetricPNG(seed: 83)
        let sharedGrainData = try asymmetricPNG(seed: 197)
        let resources = try [
            ...
            BrushPackageResource(
                id: compositeSharedGrainID,
                kind: .grain,
                mediaType: "image/png",
                data: sharedGrainData,
                pixelWidth: 64,
                pixelHeight: 64
            ),
        ].sorted { $0.id < $1.id }
        let primaryShape = BrushResourceReference(
            identifier: primaryShapeID,
            kind: .shape,
            required: true,
            fallback: nil
        )
        let secondaryShape = BrushResourceReference(
            identifier: secondaryShapeID,
            kind: .shape,
            required: true,
            fallback: nil
        )
        let sharedGrain = BrushResourceReference(
            identifier: compositeSharedGrainID,
            kind: .grain,
            required: true,
            fallback: nil
        )
        let base = try definition(
            id: "evidence.native-composite-dry",
            name: "Native Composite Dry",
            resources: [primaryShape, sharedGrain],
            shapes: [BrushShapeLayerDefinition(
                shape: .asset(primaryShapeID),
                combination: .replace,
                scale: 1,
                rotation: -0.14,
                offset: SIMD2(-0.12, 0.04)
            )],
            grains: [grain(
                .asset(compositeSharedGrainID),
                strength: 0.58
            )],
            flow: 0.76,
            hardness: 0.86,
        ...
        let primary = BrushComponentDefinition(
            identifier: BrushComponentIdentifier("dry-body"),
            ordinal: 0,
            resources: baseComponent.resources,
            coverage: baseComponent.coverage,
            placement: BrushPlacementDefinition(
                baseSpacingFraction: 0.08,
                maximumSpacingFraction: 0.22,
                baseFlow: baseComponent.placement.baseFlow,
                strokeOpacity: 0.88,
            ...
            sensorProgram: baseComponent.sensorProgram,
            emission: baseComponent.emission,
            tipSupports: baseComponent.tipSupports
        )
        var secondaryOutputs = baseComponent.sensorProgram.outputs
        ...
        let secondary = BrushComponentDefinition(
            identifier: BrushComponentIdentifier("dry-detail"),
            ordinal: 1,
            resources: [secondaryShape, sharedGrain],
            ...
            placement: BrushPlacementDefinition(
                baseSpacingFraction: 0.19,
                maximumSpacingFraction: 0.38,
                baseFlow: 0.48,
                strokeOpacity: 0.62,
                baseScatterFraction: 0,
                baseRotation: 0,
                baseJitterFraction: 0,
                baseOffset: SIMD2(2.4, -1.2)
            ),
            dynamics: baseComponent.dynamics,
            color: BrushColorBehaviorDefinition(
        ...
        return try BrushPackage(
            ...
            definition: definition,
            resourceData: [
                primaryShapeID: primaryShapeData,
                secondaryShapeID: secondaryShapeData,
                compositeSharedGrainID: sharedGrainData,
            ]
        )
    }
