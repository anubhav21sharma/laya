...
public enum LegacyBrushRecipeAdapter {
    ...
    public static func definition(from recipe: BrushRecipe, displayName: String) throws -> BrushDefinition {
        ...
        let dynamics = BrushDynamicsDefinition(
            size: map(recipe.sizeMapping, disabled: 1), flow: map(recipe.flowMapping, disabled: 1), opacity: constant(1), spacing: map(recipe.spacingMapping, disabled: 1), rotation: map(recipe.rotationMapping, disabled: 0), scatter: map(recipe.scatterMapping, disabled: 1), hardness: map(recipe.hardnessMapping, disabled: 1), grain: map(recipe.grainMapping, disabled: 1), offsetX: constantZero, offsetY: constantZero, hue: constantZero, saturation: constantZero, brightness: constantZero, secondaryColorMix: constantZero, noPressureNeutral: recipe.noPressureNeutral, randomization: recipe.randomization
        )
    ...
    public static func recipe(from definition: BrushDefinition) throws -> BrushRecipe {
        ...
        return try BrushRecipe(
            id: definition.id, schemaVersion: definition.schemaVersion, shape: definition.coverage.shapes[0].shape, grain: grain, grainCoordinateMode: coordinateMode, grainTransform: grainTransform, material: recipeMaterial(definition.material), baseSpacingFraction: definition.placement.baseSpacingFraction, maximumSpacingFraction: definition.placement.maximumSpacingFraction, baseFlow: definition.placement.baseFlow, strokeOpacity: definition.placement.strokeOpacity, baseHardness: definition.coverage.baseHardness, baseScatterFraction: definition.placement.baseScatterFraction, baseRotation: definition.placement.baseRotation, aspectRatio: definition.coverage.aspectRatio, sizeMapping: recipeMapping(definition.dynamics.size, disabled: 1), flowMapping: recipeMapping(definition.dynamics.flow, disabled: 1), spacingMapping: recipeMapping(definition.dynamics.spacing, disabled: 1), rotationMapping: recipeMapping(definition.dynamics.rotation, disabled: 0), scatterMapping: recipeMapping(definition.dynamics.scatter, disabled: 1), hardnessMapping: recipeMapping(definition.dynamics.hardness, disabled: 1), grainMapping: recipeMapping(definition.dynamics.grain, disabled: 1), noPressureNeutral: definition.dynamics.noPressureNeutral, randomization: definition.dynamics.randomization, colorAdjustment: definition.color.baseAdjustment, stabilization: definition.stabilization, taper: definition.taper, replayMode: definition.replayMode, replayLimits: definition.replayLimits
        )
    }
    ...
    private static func map(_ mapping: BrushMapping, disabled: Float) -> BrushMappingDefinition {
        switch mapping.response {
        case .disabled: return constant(disabled)
        case .linear: return BrushMappingDefinition(input: mapping.input, response: .linear, scale: mapping.outputMaximum - mapping.outputMinimum, offset: mapping.outputMinimum, lowerClamp: mapping.outputMinimum, upperClamp: mapping.outputMaximum, inverted: false, jitter: 0, missingInputValue: 1)
        case .boundedPower: return BrushMappingDefinition(input: mapping.input, response: .boundedPower(exponent: mapping.exponent), scale: mapping.outputMaximum - mapping.outputMinimum, offset: mapping.outputMinimum, lowerClamp: mapping.outputMinimum, upperClamp: mapping.outputMaximum, inverted: false, jitter: 0, missingInputValue: 1)
        }
    }
    private static func recipeMapping(_ mapping: BrushMappingDefinition, disabled: Float) throws -> BrushMapping {
        guard mapping.inverted == false, mapping.jitter == 0, mapping.missingInputValue == 1 else { throw BrushDefinitionValidationError.semanticLoss("mapping modifiers") }
        switch mapping.response {
        ...
        case .linear: guard mapping.offset == mapping.lowerClamp, mapping.scale == mapping.upperClamp - mapping.lowerClamp else { throw BrushDefinitionValidationError.semanticLoss("linear mapping") }; return BrushMapping(response: .linear, input: mapping.input, outputMinimum: mapping.lowerClamp, outputMaximum: mapping.upperClamp, exponent: 1)
        case let .boundedPower(exponent): guard mapping.offset == mapping.lowerClamp, mapping.scale == mapping.upperClamp - mapping.lowerClamp else { throw BrushDefinitionValidationError.semanticLoss("power mapping") }; return BrushMapping(response: .boundedPower, input: mapping.input, outputMinimum: mapping.lowerClamp, outputMaximum: mapping.upperClamp, exponent: exponent)
