...
public enum StageFourAnchorDefinitions {
    ...
    public static let airbrush = make(
        ...
        grains: [], flow: 0.14, hardness: 0.12, aspect: 1,
        accumulation: .flow, edge: .none
    )
    public static let eraser = make(
        id: "builtin.native-eraser", name: "Native Eraser", shape: .hardRound,
        grains: [], flow: 1, hardness: 0.92, aspect: 1,
        accumulation: .destinationOut, edge: .none
    )
    ...
    private static func make(
        ...
        aspect: Float, accumulation: BrushAccumulationMode, edge: BrushEdgeTreatment
    ) -> BrushDefinition {
        do {
            let one = constant(1)
            let zero = constant(0)
            return try BrushDefinition(
                ...
                placement: BrushPlacementDefinition(
                    ...
                    baseFlow: flow, strokeOpacity: 0.9, baseScatterFraction: 0.02,
                    baseRotation: 0, baseJitterFraction: 0.01, baseOffset: .zero
                ),
                dynamics: BrushDynamicsDefinition(
                    size: BrushMappingDefinition(
                        ...
                        inverted: false, jitter: 0, missingInputValue: 0.5
                    ...
                    secondaryColorMix: zero, noPressureNeutral: 0.5,
                    randomization: .none
                ),
                ...
                material: BrushMaterialDefinition(
                    ...
                    edgeTreatment: edge, strength: 0.9, wetness: 0,
                    bleedRadius: 0, softenPasses: 0, accumulationLimit: 0.9,
